...
def get_args_parser():
    ...
    parser.add_argument('--no_pe', action = 'store_true', help = 'not using positional encoding') # True when used
...
@torch.no_grad()
def accuracy(output, target, topk=(1,)): # 取topk看是否命中正确的分类
...
class Sparse_Backbone_Minkowski(ME.MinkowskiNetwork):
  def __init__(self):
    ...
    if(args.backbone_feature_encode): #false
      ...
      if not args.backbone_bn: # if need our instance norm because standard BasicBlock is BatchNorm
...
class MLP(nn.Module): # modified layers: more hidden layers that can be defined
...
class MLP_hn(nn.Module): #hypernets of MLP
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, input_dim_fea):
        ...
        self.layers_dims = list(zip([input_dim + 1] + h_plus, h + [output_dim])) # bias永远和output要对齐，所以加到input上
        self.layers_size = [a * b for a,b in self.layers_dims]
        #ori version
        if not flag_hidden_layer:
            self.layer = nn.Linear(input_dim_fea, sum(self.layers_size)) # sum 中的是总参数
        else:
    ...
    def forward(self, x, feature):
      ...
      net_par_layers = torch.split(net_par, self.layers_size, dim=-1) # 按照每一个layer的输入+输出+1分成layers个
      for i in range(len(self.layers_size)):
        ...
        x = torch.einsum('...ij,...jk->...ik', x, layer_par[...,:-1,:]) + layer_par[...,-1:,:] # can broadcast
...
class BackBone2VoxelTokens(nn.Module):
    ...
    def forward(self, locations, features):
        ...
        locations_batch_idx = locations[:,-1:] #identify which sample the point belongs to, how to conv
        locations = torch.cat([locations_batch_idx, locations_pos], dim=1) 
        output = self.backbone([locations, features])
        
        sparse_locations = output.C#.to(device) for Sparse tensor,C is loations,F is feature 
        ...
        sparse_locations = sparse_locations[:,1:] // output.tensor_stride[0] # 降采样出现了坐标的变化，但ME不变坐标本身
        ...
        pad_dim = max(batch_number_samples) # one instance max number of sparse voxels
        ...
        voxel_pos_embedding = self.position_encoding(sparse_locations) # voxel use sin embedding [n, 3*num_pos_feats]
        voxel_feature = torch.split(sparse_features, batch_number_samples) #[index, n,m*6]
        voxel_pos_embedding = torch.split(voxel_pos_embedding, batch_number_samples)
        input_padding_mask = torch.split(input_padding_mask, batch_number_samples) #split use batch_number_samples to split in instance
        ...
        voxel_features_padding_mask = torch.stack(input_padding_mask_list, dim=0) # mask used to indentify which is not used
        ...
        return voxel_features, voxel_position_encoding, voxel_features_padding_mask, torch.split(torch.cat((batch_idx.view(-1,1), sparse_locations), dim=-1), batch_number_samples), attention_mask
            #[seq_len(pad_im), batch_size, m*6]        [pad_dim,bs]                                          after div's voxel coord 
class DETR_Corner_Tripath(nn.Module):
    ...
    def forward(self, hs):
        ...
        if not args.no_topo:
          if(args.normalize_embed_feature): #not used?
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_coord, output_corner_topo_embedding): # 输出其他层的结果
...
class DETR_Curve_Tripath(nn.Module):
    ...
    def __init__(self, num_queries, hidden_dim,  aux_loss=False, device = None): #num_classes is not used for corner detection #backbone
        ...
        if args.no_pe: # expression is week that cannot express high freq shape
          if True:
            #no pe
            self.curve_shape_embed = MLP_hn(1, hn_mlp_dim, 3, 3, hidden_dim)
            self.curve_pe = (torch.arange(points_per_curve, dtype=torch.float32, device=device) / (points_per_curve - 1)).view(points_per_curve,1) # points_per_curve = 34, 0~33 -> 0~1 归一化参数坐标
          ...
          if args.ori_mlp:
            ...
            self.curve_pe = nn.Embedding(points_per_curve, hn_pe_dim) ## learned pe; can now i point have what features
        else:
          self.curve_shape_embed = MLP_hn(hn_pe_dim, hn_mlp_dim, 3, 3, hidden_dim)
          ...
          self.curve_pe = torch.cat([sin_mat, cos_mat], dim=1).view(points_per_curve, hn_pe_dim) #mat[p,i]=2*pi*p*base^i/(points_per_curve-1) one circle
    ...
    def forward(self, hs):
        ...
        if args.no_pe:
          if True:
            sampled_points_feature = self.curve_pe.view(1,1,1,points_per_curve, 1).repeat(1, args.batch_size, args.num_curve_queries, 1,1) # the same as parameterization_coord
        ...
        # sample_points are points that have pe
...
class DETR_Patch_Tripath(nn.Module):
    ...
    def forward(self, hs):
        """ The forward expects a NestedTensor, which consists of:
            ...
            It returns a dict with the following elements:
               ...
               - "aux_outputs": Optional, only returned when auxilary losses are activated. It is a list of
                                dictionnaries containing the two above keys for each decoder layer.
        """
        outputs_start_point_coord = self.patch_center_embed(hs).tanh()*0.5 # [-0.5,0.5] when detr sigmod
        if not flag_using_scale:
        ...
        # points 2D grid , normalize to 0~1
        #output normals works only for no_pe
        if args.no_pe:
          #no pe
          sampled_points_feature = [torch.cat([self.patch_pe_x[i], self.patch_pe_y[j]], dim = -1) for i in range(points_per_patch_dim) for j in range(points_per_patch_dim)] # the same as parameterization_coord
...
class DETR_Shape_Tripath(nn.Module):
    def __init__(self, backbone, position_encoding, tripath_transformer, num_corner_queries, num_curve_queries, num_patch_queries, aux_loss=False, device = None):
        ...
        if not args.no_tripath:
          self.primitive_type_embed = nn.Embedding(3, m*6) #corner, curve, patch; they are not the same
        else:
    ...
    def forward(self, locations, features):
        if perform_profile:
          t0 = time.time() # get time before backbone
        voxel_features, voxel_position_encoding, voxel_features_padding_mask, sparse_locations, attention_mask = self.backbone(locations, features) #feature have no idx
        if perform_profile:
          t1 = time.time()
...
def reduce_dict(input_dict, average=True):
    ...
    with torch.no_grad():
        ...
        dist.all_reduce(values) # all gpus' values add and broadcast to all gpus
        if average:
            values /= world_size # div to get average
...
def prepare_experiment_folders(exp_name): # create some needed folder
...
class BackBoneWrapper(nn.Module): # not used
...
class EncoderDecoder(nn.Module): #not used
...
def eval_pipeline(flag_eval = True):  
  ...
  device = 'cuda:0' if torch.cuda.is_available() else 'cpu' # 优先用第一个gpu
  disable_aux_loss_output = True
...
def pipeline_abc(rank, world_size):
  ...
  param_dicts = [{"params": [p for n, p in model_without_ddp.named_parameters() if p.requires_grad]}] # name value
  ...
  optimizer = torch.optim.AdamW(param_dicts, lr=args.lr, weight_decay=args.weight_decay)
  lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.lr_drop, gamma = 0.5) # step_size = args.lr_drop：每隔多少个 epoch 执行一次 lr *= gamma
  ...
  for train_iter in training_range:
    if True:
      ...
      print("corner_points:", type(corner_points), corner_points.shape)
      print("corner_batch_idx:", type(corner_batch_idx), corner_batch_idx.shape)
      print("batch_sample_id:", type(batch_sample_id))
      for idx, curve_dict in enumerate(target_curves_list):
        print(f"  Curve dict {idx}:")
      ...
      for idx, patch_dict in enumerate(target_patches_list):
        print(f"  Patch dict {idx}:")
      ...
      batch_corner_numbers = []
      for i in range(args.batch_size):
        batch_corner_numbers.append((corner_batch_idx==i).sum()) # split to fit the curve and patch list
      target_corner_points_list = torch.split(corner_points, batch_corner_numbers)
      t1 = time.time()
