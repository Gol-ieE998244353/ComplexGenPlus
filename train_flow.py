...
from flow_model import FlowConfig, BrepFlow, build_flow_system
...
def collate_fn_varlen_pc(batch):
    """拼接变长点云，添加batch_idx"""
    result = {}
    for key in ['curve_latent', 'patch_latent', 'topo_latent']:
        if key in batch[0]:
...
class FlowTrainer:
        ...
        for batch_idx, batch in enumerate(pbar):
            try:
                ...
                with autocast(enabled=self.args.use_amp):
                    m = self.model.module if self.world_size > 1 else self.model
                    losses = m.training_step(batch_gpu, self.args.time_sampling, epoch)
    ...
    def validate(self, epoch):
        self.model.eval()
        losses_sum = {'total': 0.0, 'curve_vel_loss': 0.0, 'patch_vel_loss': 0.0, 
                      'padding_loss': 0.0, 'topo_vel_loss': 0.0, 'topo_recon_loss': 0.0}  # [修改]
        n_batch = 0
        for batch in tqdm(self.val_loader, desc="Val", disable=self.rank != 0):
            try:
                ...
                with autocast(enabled=self.args.use_amp):
                    m = self.model.module if self.world_size > 1 else self.model
                    losses = m.training_step(batch_gpu, 'uniform', epoch)
        ...
        for sample_idx, sample in enumerate(samples[:actual_num]):
            try:
                ...
                for t_start in self.vis_intermediate_ts:
                    ...
                    result = m.denoise_from_t(x_t, t_start, cond_tokens, self.args.num_inference_steps)
                    sample_out = {
                ...
                # 纯生成
                if pc_feat is not None:
                    gen_out = m.generate(pc_feat, pc_batch_idx, batch_size=1, 
