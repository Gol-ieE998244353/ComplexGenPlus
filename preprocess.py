...
python preprocess.py --data_folder data/large/train --output_folder latents/ \
       --vae_checkpoint experiments/topo_class_token/ckpt/epoch_150.pth \
       --topo_ae_checkpoint experiments/topo_class_token/ckpt/epoch_150.pth  \
       --topo_hidden_dim 384 \
...
class BRepPreprocessor:
    ...
    def _load_vae(self, ckpt_path):
        ...
        def _load_component(module, key, pretty_name):
            raw_state = ckpt.get(key, {})
            if not raw_state:
                print(f"[WARN] Checkpoint lacks '{key}' weights for {pretty_name}; using random init.")
                return
            state_dict = clean(raw_state)
            incompatible = module.load_state_dict(state_dict, strict=False)
            if incompatible.missing_keys:
                preview = ', '.join(incompatible.missing_keys[:5])
                print(f"[WARN] {pretty_name} missing {len(incompatible.missing_keys)} keys (e.g., {preview}).")
            if incompatible.unexpected_keys:
                preview = ', '.join(incompatible.unexpected_keys[:5])
                print(f"[WARN] {pretty_name} has unexpected keys {len(incompatible.unexpected_keys)} (e.g., {preview}).")

        _load_component(self.curve_encoder, 'curve_encoder', 'CurveEncoder')
        _load_component(self.patch_encoder, 'patch_encoder', 'PatchEncoder')
        _load_component(self.curve_decoder, 'curve_decoder', 'CurveDecoder')
        _load_component(self.patch_decoder, 'patch_decoder', 'PatchDecoder')
    ...
    def _load_topo_ae(self, ckpt_path):
        ...
        # 获取VAE的配置参数
        from train_pc_nm import Config as VaeConfig
        vae_cfg = VaeConfig()
        
        topo_cfg = TopoConfig(
            HIDDEN_DIM=self.args.topo_hidden_dim,
            CURVE_LATENT_DIM=vae_cfg.CURVE_LATENT_DIM,
            PATCH_LATENT_DIM=vae_cfg.PATCH_LATENT_DIM
        )
        self.topo_ae = TopoVAE(topo_cfg).to(self.device).eval()  # [修改]
        ckpt = torch.load(ckpt_path, map_location=self.device)
        clean = lambda sd: {k.replace('module.', ''): v for k, v in sd.items()}
        topo_state = clean(ckpt.get('model', ckpt))
        incompatible = self.topo_ae.load_state_dict(topo_state, strict=False)
        if incompatible.missing_keys:
            preview = ', '.join(incompatible.missing_keys[:5])
            print(f"[WARN] TopoVAE missing {len(incompatible.missing_keys)} keys (e.g., {preview}).")
        if incompatible.unexpected_keys:
            preview = ', '.join(incompatible.unexpected_keys[:5])
            print(f"[WARN] TopoVAE has unexpected {len(incompatible.unexpected_keys)} keys (e.g., {preview}).")
    ...
    def encode_curves(self, curves):
        curve_mean, curve_logvar = self.curve_encoder(
            ...
            curves['labels'], curves['scale'], curves['center'], curves['mask']
        )
        # 经过to_mean和to_logvar后，padding部分可能不为0，这里强制置0以确保后续处理正确
        # 确保 padding 部分为 0
        curve_mean = curve_mean * curves['mask'].unsqueeze(-1).float()
        curve_logvar = curve_logvar * curves['mask'].unsqueeze(-1).float()
    ...
    def encode_patches(self, patches):
        patch_mean, patch_logvar = self.patch_encoder(
            ...
            patches['v_closed'], patches['labels'], patches['scale'], patches['center'], patches['mask']
        )
        # 确保 padding 部分为 0
        patch_mean = patch_mean * patches['mask'].unsqueeze(-1).float()
        return patch_mean
    ...
    def process_batch(self, batch_idx, pc_data, curves, patches, save_gt_for_vis=False):
        ...
        for i in range(B):
            ...
            curve_x0 = torch.zeros_like(curve_latent_padded)
            patch_x0 = torch.zeros_like(patch_latent_padded)
            if n_valid_curves > 0:
                curve_x0[:n_valid_curves] = torch.randn_like(curve_x0[:n_valid_curves])
            if n_valid_patches > 0:
                patch_x0[:n_valid_patches] = torch.randn_like(patch_x0[:n_valid_patches])
            result = {
                'curve_latent': curve_latent_padded.cpu(),
                'patch_latent': patch_latent_padded.cpu(),
                'curve_x0': curve_x0.cpu(),
                'patch_x0': patch_x0.cpu(),
            ...
            if topo_repr is not None:
                ...
                topo_x0 = torch.zeros_like(topo_latent_padded)
                if n_valid_topo > 0:
                    topo_x0[:n_valid_topo] = torch.randn_like(topo_x0[:n_valid_topo])
                result['topo_latent'] = topo_latent_padded.cpu()
                result['topo_x0'] = topo_x0.cpu()
